...
def update_statistics(request):
    ...
    try:
        statistics = update_channel_statistics(youtube=youtube)
    except YoutubeStatistics.DoesNotExist:
        return Response({"message": "youtube_channel_does_not_exist"}, status=400)
    
    serializer = YoutubeStatisticsSerializer(statistics)
    return Response(json.dumps(serializer.data), status=200)
